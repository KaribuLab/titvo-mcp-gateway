...
remote_state {
  ...
  config = {
    ...
    s3_bucket_tags = merge(local.serverless.locals.common_tags, {
      Name = local.serverless.locals.service_bucket
    })
    dynamodb_table_tags = merge(local.serverless.locals.common_tags, {
      Name = "${local.serverless.locals.service_bucket}-tfstate-lock"
    })
  }
}
